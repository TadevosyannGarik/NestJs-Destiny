...
export const GET = async ( req: NextRequest, { params }: { params: { challengeOptionId: number }}) => {
    if (!isAdmin()) {
...
export const PUT = async ( req: NextRequest, { params }: { params: { challengeOptionId: number }}) => {
    if (!isAdmin()) {
        return new NextResponse("Unauthorized", { status: 401 })
    }
